...
const (
	fileGeneratedResolvConfContentHeader         = "# Generated by R-VPN"
...
# The original file can be restored from ` + fileDefaultResolvConfBackupLocation + "\n\n"
	...
	fileDefaultResolvConfBackupLocation = defaultResolvConfPath + ".original.rvpn"
...
func (f *fileConfigurator) updateConfig(nbSearchDomains []string, nbNameserverIP netip.Addr, cfg *resolvConf, stateManager *statemanager.Manager) error {
	...
	log.Infof("created a R-VPN managed %s file with the DNS settings. Added %d search domains. Search list: %s", defaultResolvConfPath, len(searchDomainList), searchDomainList)
...
func (f *fileConfigurator) restoreUncleanShutdownDNS(storedDNSAddress netip.Addr) error {
	...
	// current address is still rvpn's non-available dns address -> restore
	currentDNSAddress := resolvConf.nameServers[0]
	if currentDNSAddress == storedDNSAddress {
		return restoreResolvConfFile()
	}

	log.Infof("restoring unclean shutdown: first current nameserver differs from saved nameserver pre-rvpn: %s (current) vs %s (stored): not restoring", currentDNSAddress, storedDNSAddress)
	return nil
}
