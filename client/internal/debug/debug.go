...
status.txt: Anonymized status information of the R-VPN client.
client.log: Most recent, anonymized client log file of the R-VPN client.
rvpn.err: Most recent, anonymized stderr log file of the R-VPN client.
rvpn.out: Most recent, anonymized stdout log file of the R-VPN client.
...
config.txt: Anonymized configuration information of the R-VPN client.
network_map.json: Anonymized sync response containing peer configurations, routes, DNS settings, and firewall rules.
state.json: Anonymized client state dump containing rvpn states for the active profile.
...
Domains
All domain names (except for the rvpn domains) are replaced with randomly generated strings ending in ".domain". Anonymized domains are consistent across all files in the bundle.
...
State File
The state.json file contains anonymized internal state information of the R-VPN client, including:
...
The goroutine, block, mutex, and heap profiling files contain process information that might help the R-VPN team diagnose performance or memory issues. The information in these files doesn't contain personal data.
...
- Table: Routing table name (main, local, rvpn, etc.)
...
Resolved Domains
The resolved_domains.txt file contains information about domain names that have been resolved to IP addresses by R-VPN's DNS resolver. This includes:
...
Configuration
The config.txt file contains anonymized configuration information of the R-VPN client. Sensitive information such as private keys and SSH keys are excluded. The following fields are anonymized:
- ManagementURL
- AdminURL
- NATExternalIPs
- CustomDNSAddress
...
- Table: Target routing table name (main, local, rvpn, etc.)
...
const (
	...
	errorLogFile  = "rvpn.err"
	stdoutLogFile = "rvpn.out"

	darwinErrorLogPath  = "/var/log/rvpn.out.log"
	darwinStdoutLogPath = "/var/log/rvpn.err.log"
)
...
func (g *BundleGenerator) Generate() (resp string, err error) {
	bundlePath, err := os.CreateTemp("", "rvpn.debug.*.zip")
	if err != nil {
...
func (g *BundleGenerator) addCommonConfigFields(configContent *strings.Builder) {
	configContent.WriteString("R-VPN Client Configuration:\n\n")
