...
var statusCmd = &cobra.Command{
	Use:   "status",
	Short: "Display R-VPN client status",
	Long:  "Display the current status of the R-VPN client, including connection status, peer information, and network details.",
	RunE:  statusFunc,
}
...
func init() {
	...
	statusCmd.PersistentFlags().BoolVar(&ipv4Flag, "ipv4", false, "display only R-VPN IPv4 of this peer, e.g., --ipv4 will output 100.64.0.33")
	statusCmd.MarkFlagsMutuallyExclusive("detail", "json", "yaml", "ipv4")
	statusCmd.PersistentFlags().StringSliceVar(&ipsFilter, "filter-by-ips", []string{}, "filters the detailed output by a list of one or more IPs, e.g., --filter-by-ips 100.64.0.100,100.64.0.200")
	statusCmd.PersistentFlags().StringSliceVar(&prefixNamesFilter, "filter-by-names", []string{}, "filters the detailed output by a list of one or more peer FQDN or hostnames, e.g., --filter-by-names peer-a,peer-b.rvpn.cloud")
...
func statusFunc(cmd *cobra.Command, args []string) error {
	...
	if status == string(internal.StatusNeedsLogin) || status == string(internal.StatusLoginFailed) ||
		...
		cmd.Printf("Daemon status: %s\n\n"+
			...
			"you can use a setup-key:\n\n rvpn up --management-url <YOUR_MANAGEMENT_URL> --setup-key <YOUR_SETUP_KEY>\n\n"+
			"More info: https://docs.rsoftware.net/how-to/register-machines-using-setup-keys\n\n",
			resp.GetStatus(),
		)
		return nil
	}
