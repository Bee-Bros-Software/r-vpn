...
func TestServiceLifecycle(t *testing.T) {
	...
	serviceName = "rvpntest" + fmt.Sprintf("%d", time.Now().Unix())
	defer func() {
		serviceName = originalServiceName
	}()

	tempDir := t.TempDir()
	configPath = fmt.Sprintf("%s/rvpn-test-config.json", tempDir)
	logLevel = "info"
	daemonAddr = fmt.Sprintf("unix://%s/rvpn-test.sock", tempDir)
